...
function StreamBar(props) {
  ...
  return (
    <div className={`${props.className}`}>
      {isLocalStream && (
        <div className={`${styles["network-quality-container"]}`}>
          <img src="up.png" className={styles.arrow}></img>
          <div className={`${styles["network-quality"]}`}>
            {[...new Array(5).keys()].map((index) => (
              <div
                key={`uplink-${index}`}
                className={`${styles[`network-quality-${index + 1}`]} ${
                  config.uplinkNetworkQuality > index ? styles.green : ""
                }`}
              ></div>
            ))}
          </div>
          ...
          <div className={`${styles["network-quality"]}`}>
            {[...new Array(5).keys()].map((index) => (
              <div
                key={`downlink-${index}`}
                className={`${styles[`network-quality-${index + 1}`]} ${
                  config.downlinkNetworkQuality > index ? styles.green : ""
                }`}
              ></div>
            ))}
          </div>
      ...
      <div className={`${styles.bar}`}>
        {/* 显示用户名 */}
        <span className={`${styles["bar-name"]}`}>{config.userID}</span>
          ...
          <div className={`${styles["bar-icon"]} ${styles[config.type]}`}>
            <div className={styles["bar-item"]}>
                ...
                >
                  {config.hasVideo && !config.mutedVideo ? (
                    <VideocamIcon
                      ...
                      onClick={(e) => handleChange("video", e)}
                      style={style}
                    />
                  ) : (
                    <VideocamOffIcon
              ...
              {setting.hasAudio && isLocalStream && (
                <Tooltip
                  title={config.mutedAudio ? "Unmute Audio" : "Mute Audio"}
                  arrow
                >
                  {config.hasAudio && !config.mutedAudio ? (
                    <div
                      className={`${styles["audio-volume-container"]} ${styles.pointer}`}
                      onClick={(e) => handleChange("audio", e)}
                      style={style}
                    >
                      <MicIcon className={styles["audio-icon"]} />
                      <div
                        className={styles["volume-container"]}
                        style={{ height: `${config.audioVolume * 4}%` }}
                      >
                        <MicIcon
                          className={styles["green-audio-icon"]}
                          style={{ color: "#1afa29" }}
                        />
                      </div>
                    </div>
                ...
                >
                  {config.hasVideo && !config.mutedVideo && (
                    <PictureInPictureIcon
                      className={styles.pointer}
                      onClick={(e) => handleChange("picture", e)}
                      style={style}
                    />
                  )}
                </Tooltip>
              ...
              {setting.hasAudio &&
                ...
                (config.hasAudio && !config.mutedAudio ? (
                  <div
                    className={`${styles["audio-volume-container"]}`}
                    style={style}
                  >
                    <MicIcon className={styles["audio-icon"]} />
                    <div
                  ...
                  {setting.subscribedVideo && (
                    ...
                    >
                      {config.subscribedVideo ? (
                        <VisibilityIcon
                          className={styles.pointer}
                          onClick={(e) => handleChange("subscribedVideo", e)}
                          style={style}
                        />
                      ) : (
                        <VisibilityOffIcon
                  ...
                  {setting.subscribedAudio && (
                    ...
                    >
                      {config.subscribedAudio ? (
                        <VolumeUpIcon
                          className={styles.pointer}
                          onClick={(e) => handleChange("subscribedAudio", e)}
                          style={style}
                        />
                      ) : (
                        <VolumeOffIcon
        ...
        {setting.subscribedVideo && isRemoteAuxStream && (
          <FullscreenIcon
            onClick={(e) => handleChange("full", e)}
            style={style}
          />
        )}
      </div>
